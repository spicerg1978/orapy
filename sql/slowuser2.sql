...
prompt -- ========================================
prompt -- Waited on since last login?
prompt -- ========================================
...
select s.sql_text      SQ,
       rpad('| '||substr(lpad(' ',1*(depth))||operation|| decode(options, null,'',' '||options), 1, 59), 60, ' ')||'|'||
       rpad(substr(object_name||' ',1, 30), 30, ' ')||'|' OP,
