create or replace PACKAGE pkg_query_manager AS
/***********************************************************************************************************************
 Change History:
...
create or replace PACKAGE BODY pkg_query_manager AS
/***********************************************************************************************************************
 Change History:
 ...
 13/12/10      Gareth Spicer		Amended the GET_HALF_TRADES to use optimizer_index_cost_adj hint for RT queries
...
***********************************************************************************************************************/
   ...
   BEGIN
      ...
      v_sql := 'INSERT INTO half_trade_results '||'SELECT ';


      IF UPPER(p_where_clause) LIKE '%''RT''%' THEN
        v_sql := v_sql||'/*+ opt_param(''optimizer_index_cost_adj'',20) */';
      END IF;

      v_sql := v_sql||' :p_query_id, rownum AS query_row_num, ht.* FROM ('||'SELECT * FROM half_trade WHERE ';
